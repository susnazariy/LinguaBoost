<template>
  <div class="task-form">
    <h3 class="task-title">Виправлення речення</h3>
  ...
  methods: {
    async fetchTask() {
      try {
        ...
        this.sentence = response.data[0].correct_text;
        this.fixedSentence = response.data[0].bad_text;
      } catch (error) {
    ...
    async updateTask() {
      ...
      const data = {
        fix_sentence_task_id: this.id,
        new_correct_text: this.sentence,
        new_bad_text: this.fixedSentence,
      };

      try {
        const token = this.$store.state.token;
        await axios.post(
          "http://127.0.0.1:8000/update_fix_sentence_task/",
          data,
          {
            headers: { Authorization: `Token ${token}` },
          }
    ...
    async deleteTask(task) {
      ...
      try {
        const response = await axios.post(
          "http://127.0.0.1:8000/delete_fix_sentence_task/",
          { fix_sentence_task_id: task },
          {
            headers: {
        ...
        if (response.data === "Fix sentence task successfully deleted!") {
          Swal.fire({
            icon: "success",
        ...
        } else if (response.data === "Delete fix sentence task error!") {
          Swal.fire({
            icon: "error",
            title: "Помилка!",
