...
export default {
  ...
  methods: {
    ...
    async updateExercise() {
      ...
      formData.append("reading_exercise_id", this.exerciseId);
      ...
      try {
        const token = this.$store.state.token;
        await axios.post(
          "http://127.0.0.1:8000/update_reading_exercise/",
          formData,
          {
            headers: {
    ...
    async deleteExercise() {
      ...
      try {
        const response = await axios.post(
          "http://127.0.0.1:8000/delete_reading_exercise/",
          { reading_exercise_id: this.exerciseId },
          {
            headers: {
              Authorization: `Token ${token}`,
            },
        ...
        if (response.data === "Reading exercise successfully deleted!") {
          Swal.fire({
            icon: "success",
